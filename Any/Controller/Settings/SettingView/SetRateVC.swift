...
extension SetRateVC {
        ...
        do {
            ...
            if(swiftyJsonVar["status"].stringValue == "1") {
                if self.arr_AllSpecificRate.count > 0 {
                    Task {
                        await self.WebUpdateDateShifts()
                    }
                } else {
    ...
    func WebUpdateDateShifts() async {
        showProgressBar()
        var paramsDict:[String:AnyObject] = [:]
        paramsDict["user_id"]  =   USER_DEFAULT.value(forKey: USERID) as AnyObject
    ...
    func WebDeleteDateRate(_ dateiD:String) async {
        showProgressBar()
        var paramsDict:[String:AnyObject] = [:]
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if collectionView == weeklyRateCollectionVw {
            ...
            cell.cloDelete = { [self] in
                Task {
                    await self.WebDeleteDateRate(obj["id"].stringValue)
                }
            }
        }
        return cell
