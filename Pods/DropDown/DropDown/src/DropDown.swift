...
extension DropDown {
    ...
    fileprivate func fittingWidth() -> CGFloat {
        if templateCell == nil {
            guard let cell = cellNib.instantiate(withOwner: nil, options: nil).first as? DropDownCell else {
                assertionFailure("DropDown: cellNib must contain a DropDownCell as first object")
                return 0
            }
            templateCell = cell
        }
